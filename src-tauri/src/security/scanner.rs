...
impl SecurityScanner {
        ...
        while let Some(next) = iter.next() {
            ...
            if entry.file_type().is_dir() {
                if let Some(name) = entry.file_name().to_str() {
                    if SKIP_DIR_NAMES.contains(&name) {
                        eprintln!("Skipping directory: {:?}", entry.path());
            ...
            if entry.file_type().is_symlink() {
                ...
                total_hard_trigger_issues.push(
                    format!(
                        "SYMLINK (File: {}): symbolic link detected inside skill directory",
                        &rel_str
                    )
                );
    ...
    pub fn scan_file(&self, content: &str, file_path: &str, _locale: &str) -> Result<SecurityReport> {
        ...
        let hard_trigger_issues: Vec<String> = hard_trigger_matches.iter()
            .map(|m| format!("{} (File: {}, Line: {}): {}",
                &m.rule_name, file_path, m.line_number, &m.description))
            .collect();
...
"#;
        ...
        assert!(report.hard_trigger_issues[0].contains("RM_RF_ROOT") ||
                report.hard_trigger_issues[0].contains("rm -rf"),
...
"#;
        ...
        assert!(report.hard_trigger_issues.iter().any(|i|
            i.contains("CURL_PIPE_SH") || i.contains("curl") || i.contains("pipe")),
