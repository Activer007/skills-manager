 ...
 *
 * 用于可视化展示 Skill 的安全等级和评分，支持多种尺寸和显示模式，支持国际化。
 *
 * @example
 * ```tsx
 * // 完整显示（带标签和评分）
...
export const TrustShield = ({
  ...
  // 获取翻译后的标签
  const label = t(`trustShield.${level}`);
  const scoreText = score ?? 'N/A';
  const ariaLabel = t('trustShield.ariaLabel', { level: label, score: scoreText });
  const titleText = t('trustShield.scoreLabel', { score: scoreText });
